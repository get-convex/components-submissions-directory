...
- Relative and root-relative README links resolved to the app origin (404) instead of GitHub (2026-06-26 11:57 UTC)
  - Root cause: `resolveRepositoryMarkdownHref` in `src/lib/markdownLinks.ts` early-returned any href starting with `/`, so a root-relative README link like `[example/README.md](/example/README.md)` (used by `@exalabs/convex-exa`) was left untouched and the browser resolved it against the app origin → `https://www.convex.dev/example/README.md` (404). GitHub instead treats a leading-`/` link in a README as repo-root-relative. A second latent bug: the base ref was hardcoded to `/blob/main/README.md`, which 404s for repos whose default branch is `master` (verified `blob/main/...` → 404, `blob/master/...` → 200 for this repo).
  - Fix: both relative and root-relative links now resolve against the repository root using `https://github.com/<owner>/<repo>/blob/HEAD/...`. `HEAD` resolves to the default branch regardless of `main` vs `master` (verified `blob/HEAD/example/README.md` → 200). Extracted a shared `parseGitHubSlug` helper and applied the same root-relative handling to `resolveRepositoryImageSrc` (images now resolve against `raw.githubusercontent.com/<owner>/<repo>/HEAD/`). Anchors (`#...`), absolute URLs (`http:`, `mailto:`, etc.), and protocol-relative (`//...`) links are still passed through unchanged.
  - Files: `src/lib/markdownLinks.ts`
  - Verification: `ReadLints` clean; Node check confirmed `/example/README.md`, `example/README.md`, and `./docs/x.md` all resolve to `https://github.com/exa-labs/exa-convex/blob/HEAD/...` while anchors/absolute/protocol-relative URLs pass through unchanged.
