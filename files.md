...
Known gap (2026-09-21): `fetchGitHubRepo` still parses github.com only, so GitLab repositories reach the review step and return an error at the repo fetch. The planned `RepoFileClient` extraction (GitHub client unchanged, GitLab client on `convex/gitlabApi.ts`) is written out in `prds/gitlab-repo-support.md`; edits to this file are currently blocked by a lint hook false positive on the literal text `query(` inside two prompt strings.

### `convex/gitlabApi.ts`

GitLab REST v4 read adapter for gitlab.com. Plain helpers with the same return shapes as the GitHub code paths so callers dispatch on provider without changing their own types: `fetchGitLabFile` (raw file at `ref=HEAD` by default), `fetchGitLabTree` (directory listing, 404 treated as empty), `fetchGitLabIssues` (mapped to the `githubIssueValidator` shape, `hasMore` from `x-next-page`), `fetchGitLabIssueCounts` (open and closed from the `x-total` header of a one item page, since gitlab.com omits `open_issues_count` for unauthenticated callers), and `fetchGitLabNamespaceAvatar` (namespace then project avatar; relative `/uploads/...` paths resolved against the host). `fillGitLabAuthorAvatar` is an internal action that submit and admin auto-fill schedule after the package row exists, writing through `_setAuthorAvatar` so mutations stay fetch free. Optional `GITLAB_TOKEN` is sent as `PRIVATE-TOKEN`; a 401 retries unauthenticated and a 429 throws a rate limit error. Project ids are `encodeURIComponent(projectPath)` so nested groups work.
