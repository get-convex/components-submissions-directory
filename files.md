...
### `vite.config.ts`
Vite build configuration. Sets up React plugin, path aliases, and includes Chef development tools injection for screenshot capabilities during development.
...
### `index.html`
Main HTML entry point. Loads the React app and CSS.

## Convex Backend Files

### `convex/schema.ts`
Database schema definition. Defines the `packages` table with all package fields including submitter information (name, email, Discord), review status (pending, in_review, approved, changes_requested, rejected), visibility (visible, hidden, archived), featured flag, and AI review fields (aiReviewStatus, aiReviewSummary, aiReviewCriteria, aiReviewedAt, aiReviewError). Also defines `packageNotes` table for threaded admin notes, `packageComments` table for public comments, and `adminSettings` table for AI automation preferences. Includes indexes for name lookup, submission date sorting, review status filtering, visibility filtering, admin settings lookup, and full-text search on both name and description. Also includes auth tables from Convex Auth.
...
- `submitPackage`: Action that validates URL, checks for duplicates, fetches data, and inserts package with submitter information
- `addPackage`: Mutation that inserts or updates package in database with submitter info
...
- `searchPackages`: Query with full-text search on visible packages (searches name and description)
...
- `updateReviewStatus`: Admin mutation to change package review status
...
- `addPackageNote`: Mutation to add a note or reply to a package
...
- `addPackageComment`: Mutation to add a public comment to a package
- `deletePackageComment`: Mutation to delete a public comment
- `updateAiReviewStatus`: Mutation to update AI review status (for "reviewing" state)
...
- Package submission form with submitter information fields (name, email, optional Discord)
...
- Admin dashboard with statistics cards (total packages, downloads, last submission)
