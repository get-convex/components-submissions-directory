...
import { resolveRepositoryImageSrc } from "../lib/markdownLinks";
 ...
 */
export function createMarkdownComponents(opts?: { repositoryUrl?: string }) {
  const repositoryUrl = opts?.repositoryUrl;
  return {
    ...
    img({ src, alt }: { src?: string; alt?: string }) {
      ...
      if (resolved && /\.(mp4|webm|mov)(\?.*)?$/i.test(resolved)) {
        return (
          <video
            src={resolved}
            controls
